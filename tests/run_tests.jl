...
function run_tests()
    ...
    for p in probs
        ...
        elapsed_time = @elapsed begin
            is_sol_valid, x, λ, iter_count, status = solve_bop(bop; max_iter=50, x_init=p.xy_init, verbosity=5, tol=1e-7, norm_dv_len=10, conv_dv_len=1, is_checking_min=true, is_checking_x_agree=true, init_solver="IPOPT", solver="IPOPT")
        end
