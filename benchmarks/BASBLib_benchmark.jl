...
```
"""
function benchmark_BASBLib(; example_ids=1:length(BASBLib.examples), verbosity=0, tol=1e-7, init_solver="IPOPT", solver="IPOPT", max_iter=50, conv_dv_len=1, do_force_hp_init=false, do_check_x_agreem=true, max_rand_restart_ct=10, rng=MersenneTwister(), do_force_dry_run=true, rating_tol=1e-3)
    dataframes = []
    success_arr = Bool[]
    elapsed_arr = Float64[]
    prob_count = 0
