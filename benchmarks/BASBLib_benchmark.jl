...
```
"""
function benchmark_BASBLib(; example_ids=1:length(BASBLib.examples), verbosity=0, tol=1e-7, init_solver="IPOPT", solver="IPOPT", max_iter=50, conv_dv_len=1, do_force_hp_init=false, do_check_x_agreem=true, max_rand_restart_ct=0, rng=MersenneTwister(), do_force_dry_run=false, rating_tol=1e-3)
    dataframes = []
    success_arr = Bool[]
    elapsed_arr = Float64[]
    prob_count = 0
    ...
    for id in example_ids
        ...
        if do_force_dry_run
            is_sol_valid, x, λ, iter_count, status, worst_fol_cond, worst_sbop_cond = solve_bop(bop; x_init, verbosity=0, tol, init_solver, solver, max_iter, conv_dv_len, do_force_hp_init, do_check_x_agreem, max_rand_restart_ct)
        end
