    ...
    while !is_converged
        ...
        for (i, J) in enumerate(follow_feas_Js)
            ...
            if is_solved
                ...
                if verbosity > 2
                    print("SBOP$i: Solved (nc: $is_SBOPi_nec sc: $is_SBOPi_suf. follower nc: $is_fol_nec sc: $is_fol_suf) J1: $(J[1]) J2: $(J[2]) x=$(round.(v[bop.inds.v["x"]],sigdigits=2)), λ=$(round.(v[bop.inds.v["λ"]],sigdigits=2))\n")
                end
