"""
Plan 2025-06-16: 
1. separate solver interfaces, make licensed components explicit, for now:
    1. IPOPT
    2. PATH
    3. HiGHS
2. define problem in construct_bop(), change BilevelOptProb structure to include
    1. solve_follower_nlp
    2. solve_BOPᵢ_nlp
    3. solve_BOPᵢ_feasibility
    4. solve_bilevel_feasibility
...
"""
function generate_derivatives(n₁, n₂, F, G, f, g; verbosity=0)
    nₓ = n₁ + n₂ # length(x)
    x_dummy = zeros(nₓ)
    m₁ = length(G(x_dummy))
    m₂ = length(g(x_dummy))
...
function construct_bop(n₁, n₂, F, G, f, g; verbosity=0)
    ...
    #(_, derivs) = generate_derivatives(n₁, n₂, F, G, f, g)

    ∇ₓ₂f = Symbolics.gradient(f_sym, x₂) #   derivs.∇ₓf[n₁+1:end]
    ∇ₓ₂g = Symbolics.sparsejacobian(g_sym, x₂) #    derivs.∇ₓg[:, n₁+1:end]
...
___________________________________
"""
function check_Λ_LP_feas(bop, v, Ji_bounds; primal_feas_tol=1e-6, zero_tol=1e-3, verbosity=0)
