...
function solve(epec, θ; tol=1e-6, max_iters=10)
    ...
    while !converged
		...
		"""
		2025-05-08
		I think there's a bug here, while it doesn't matter at initialization, for the bilevel problem this will return bilevel infeasible points -- unless we assume for any follower solution there exists a leader solution. This is only true if there are no coupling constraints, or all coupling constraints also appear in the follower's constraints via linking variables)

		In other words, we might have fucked up by not adding a if !is_initialized here...
		"""
