function w=NetworkDesignP2(x,y,keyf,keyxy)
% This file provides all functions defining DesignCentringP4 problem 
% and their first and second order derivatives.
% [dim_x dim_y dim_G dim_g] = [5 5 5 6]   
 
if nargin<4 || isempty(keyxy)
    switch keyf
    case 'F'; xy = y./(1+x); 
              w  = [50+xy(1) 10*xy(2) 10+xy(3) 10*xy(4) 50+xy(5)]*y+sum(x) ;
    case 'G'; w  = -x-1;
    case 'f'; xy = y./(1+x)/2; 
    ...
    case 'F'
        a=[2;20;2;20;2];
        switch keyxy
        case 'x' ; w = -(a/2).*(y.*y./(1+x).^2)+ones(5,1);         
