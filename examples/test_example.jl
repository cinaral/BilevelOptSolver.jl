...
#p = mb_2007_06()
mb_2007_03
include("../test/lp-qp/mb_2007_03.jl")
p = mb_2007_03()
bop, syms = construct_bop(p.n1, p.n2, p.F, p.G, p.f, p.g; verbosity=0, np=0)
#x_init = randn(MersenneTwister(456), p.n1 + p.n2)
x_init = [0.01]
is_sol_valid, x, λ, iter_count, status = solve_bop(bop; max_iter=50, x_init, verbosity=5, tol=1e-7, conv_dv_len=3, is_checking_x_agree=true, is_always_hp=false, is_nonstrict_ok=true, init_solver="IPOPT", solver="PATH")
...
#include("../test/lp-lp/ct_1982_01.jl")
#p = ct_1982_01()
#bop, syms = construct_bop(p.n1, p.n2, p.F, p.G, p.f, p.g; verbosity=0, np=0)
#x_init = randn(MersenneTwister(456), p.n1 + p.n2)
#is_sol_valid, x, λ, iter_count, status = solve_bop(bop; max_iter=50, x_init, verbosity=5, tol=1e-7, norm_dv_len=10, conv_dv_len=1, is_checking_x_agree=true, is_forcing_inactive_inds=false, is_always_hp=false, init_solver="IPOPT", solver="IPOPT")
...
#include("../test/lp-qp/as_1984_01.jl")
#p = as_1984_01()
...
#include("../test/qp-qp/as_1981_01.jl")
#p = as_1981_01()
